...
def min_max(board, player, depth, item):
  ...
  for move in moves:
    ...
    moveScore = memoizedMin(new_board, depth, toggle_player[player], item)
...
memoizedMin = memoize(min_val)
memoizedMax = memoize(max_val)
...
def min_val(board, depth, player, item):
  ...
  for move in moves:
    new_board = moveBoard(board, move, player)
    move_scores.append(memoizedMax(new_board, depth, toggle_player[player], item))
...
def max_val(board, depth, player, item):
  ...
  for move in moves:
    ...
    move_scores.append(memoizedMin(board, depth,toggle_player[player],item))
